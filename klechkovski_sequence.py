def klechkovski(n,l = None):
    ...
    quantum_hash = {'s': 0, 'p': 1, 'd': 2, 'f': 3, "g":4}
    reversed_quantum_hash = {v: k for k, v in quantum_hash.items()}
    klechkovski_sequence = []
    if not l:
        l = n - 1 #Sets l to the maximum that the max shell can handle, l starts at 0
    for shell in range(1,(n + l) + 1):
        for subshell in range(shell):
            if subshell in reversed_quantum_hash:
                ...
                #If an element's TRUE power is smaller than the MAX power, then add
                if priority <= (n + l): #n + l is the TRUE power of an element, Klechkowski sorts by that
...
klechkovski_sequence = []
for orbital in klechkovski(4):
    klechkovski_sequence.append(orbital[2])
print(" ".join(klechkovski_sequence))